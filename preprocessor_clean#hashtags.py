if __name__ == "__main__": # sort of like with MPI, we need this to do multiprocessing on windows
    ...
    pickle.dump(tf_idf.get_feature_names(), open('TF_IDF_feature_names_hashtags.pkl', 'wb'))
    ...
    topic_model = NMF(n_components=50,verbose=1,tol=.001,alpha=.1,l1_ratio=.2)
    ...
    pickle.dump(text_topic_model_WH, open('NMF_topics_WH_hashtags.pkl','wb'), protocol=4) # Save it to
    pickle.dump(topic_model, open('NMF_hashtags.pkl','wb'), protocol=4)
