...
if __name__ == "__main__": # sort of like with MPI, we need this to do multiprocessing on windows
    ...
    tf_idf = CountVectorizer(analyzer='char_wb',ngram_range=(2,4))
    ...
    pickle.dump(tf_idf.get_feature_names(), open('TF_IDF_feature_names_lang.pkl', 'wb'))
    ...
    topic_model = NMF(n_components=8,verbose=1,tol=.001,alpha=.1,l1_ratio=.2)
    #topic_model = LatentDirichletAllocation(n_topics=1000,n_jobs=-1,doc_topic_prior=50/1000.0, topic_word_prior=.1, verbose=100,batch_size=int(len(raw_text)/100),max_doc_update_iter=1000,mean_change_tol=.0001,learning_offset=30)
    ...
    pickle.dump(text_topic_model_WH, open('NMF_topics_WH_lang.pkl','wb'), protocol=4) # Save it to
    pickle.dump(topic_model, open('NMF_lang.pkl','wb'), protocol=4)
